...
def create_scaling_graph(subject_name, output_path):
    ...
    plt.scatter(X, y, color='#dc3545', s=80, alpha=0.8, label='UAC Data Points', zorder=5, edgecolors='white', linewidth=1)
    
    # Plot polynomial regression curve with consistent color
    plt.plot(X_smooth, y_smooth, color='#007bff', linewidth=3, label='Polynomial Regression (Degree 4)', alpha=0.9)
    ...
    plt.title(f'{subject_name} - Scaling Model', fontsize=16, fontweight='bold', pad=20, color='#333333')
...
def main():
    ...
    for subject in subjects_to_graph:
        filename = subject.lower().replace(' ', '_').replace('&', 'and') + '_scaling.png'
