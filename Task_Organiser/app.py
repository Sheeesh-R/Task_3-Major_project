...
def create_app() -> Flask:
    ...
    with app.app_context():
        ...
        try:
            ...
            if "units" not in columns:
                db.execute(
                    "ALTER TABLE subjects ADD COLUMN units INTEGER NOT NULL DEFAULT 2"
                )
    ...
    def priority_class(priority):
        classes = {"high": "danger", "medium": "warning", "low": "info"}
...
def register_routes(app: Flask) -> None:
    """Register all Flask route handlers on the provided `app` instance."""
        ...
        try:
            ...
            for subject in subjects_list:
                ...
                task_count = db.execute(
                    "SELECT COUNT(*) as count FROM tasks WHERE user_id = ? AND subject_id = ?",
                    (g.user["id"], subject["id"]),
                ).fetchone()
                subject["task_count"] = task_count["count"] if task_count else 0

                # Count completed tasks
                completed_count = db.execute(
                    'SELECT COUNT(*) as count FROM tasks WHERE user_id = ? AND subject_id = ? AND status = "completed"',
                    (g.user["id"], subject["id"]),
                ).fetchone()
                ...
                subject["priority_task"] = (
                    dict(priority_task) if priority_task else None
                )
        ...
        if subject_id:
            ...
            if current_subject:
                ...
                for i, result in enumerate(assessment_results):
                    result["percentage"] = (
                        round((result["raw_mark"] / result["max_mark"]) * 100, 1)
                        if result["max_mark"] > 0
                        else 0
                    )
    ...
    def add_task():
        ...
        subjects = db.execute(
            "SELECT * FROM subjects WHERE user_id = ? ORDER BY name", (g.user["id"],)
        ).fetchall()
    ...
    def delete_completed_tasks():
        ...
        try:
            ...
            if completed_count > 0:
                ...
                db.commit()
                flash(
                    f"Successfully deleted {completed_count} completed task(s)!",
                    "success",
                )
            else:
    ...
    def subjects():
        ...
        try:
            ...
            if "units" in columns and "target_mark" in columns:
                ...
                subjects = db.execute(
                    "SELECT id, name, user_id FROM subjects WHERE user_id = ? ORDER BY name",
                    (g.user["id"],),
                ).fetchall()
            ...
            for subject in subjects:
                ...
                if assessment_results:
                    ...
                    for result in assessment_results:
                        percentage = (
                            (result["raw_mark"] / result["max_mark"]) * 100
                            if result["max_mark"] > 0
                            else 0
                        )
                    ...
                    subject_dict["estimated_mark"] = (
                        round(total_weighted_score, 1) if total_weight > 0 else 0
                    )
                else:
                    subject_dict["estimated_mark"] = 0
    ...
    def add_subject():
        ...
        try:
            ...
            app.logger.debug(
                "subject_name=%s, units=%s, target_mark=%s",
                subject_name,
                units,
                target_mark,
            )
            ...
            if "UNIQUE constraint failed: subjects.name" in error_msg:
                msg = (
                    f'Subject "{subject_name}" already exists in the system. '
    ...
    def delete_subject(subject_id):
        ...
        try:
            ...
            task_count = db.execute(
                "SELECT COUNT(*) FROM tasks WHERE subject_id = ?", (subject_id,)
            ).fetchone()[0]

            if task_count > 0:
                msg = (
                    f'Cannot delete subject "{subject["name"]}" because it has '
                    f'{task_count} associated task(s). Please delete or reassign '
                    "the tasks first."
                )
                flash(msg, "error")
                return redirect(url_for("subjects"))
        ...
        if request.method == "POST":
            ...
            if error is None:
                try:
                    target_mark_value = (
                        int(target_mark)
                        if target_mark and target_mark.strip() != ""
                        else None
                    )
                    db.execute(
    ...
    def atar():
                ...
                for subject in user_subjects:
                    ...
                    if mark_input and mark_input.strip():
                        try:
                            ...
                            if (
                                "Extension" in subject["name"]
                                and subject["name"] != "Mathematics Extension 2"
                            ):
        ...
        latest_prediction = db.execute(
            """SELECT * FROM atar_predictions WHERE user_id = ? ORDER BY prediction_date DESC LIMIT 1""",
            (g.user["id"],),
        ).fetchone()

        estimated_atar = (
            latest_prediction["atar_score"]
            if latest_prediction and latest_prediction["atar_score"]
            else 0
        )
    ...
    def subject_marks(subject_id):
        ...
        for result in assessment_results:
            ...
            result_dict["weighted_score"] = round(
                result_dict["percentage"] * (result["weight"] / 100), 1
            )
    ...
    def add_assessment_result(subject_id):
        ...
        try:
            ...
            if not assessment_name:
                ...
                flash(
                    "Marks must be positive and max mark must be greater than 0",
                    "error",
                )
    ...
    @app.route(
        "/subjects/<int:subject_id>/marks/<int:result_id>/delete", methods=["POST"]
    )
    @login_required
    def delete_assessment_result(subject_id, result_id):
        ...
        try:
            ...
            """,
                (result_id, subject_id, g.user["id"], g.user["id"]),
            ).fetchone()
...
def handle_500(error):
    """Render friendly 500 page and log exception."""
